...
## Install randomForestSRC at first
...
FHS_mods_dir="/yourpath/fhs_randomForest_mods/"
...
FHS_mod = readRDS(c_mod_dir)

## please check varible names in FHS_mod$xvar.names, and make sure your data including all of variables  (using the same name!) as FHS_mod$xvar.names
v.pred <- predict(FHS_mod,Pheno_DNAm)
riskScore = v.pred$predicted
...
write.table(results_allcause, "aric_EA_randomForestSRC_allcause.csv",row.names=F,quote=F, sep=",")
write.table(results_CVD, "aric_EA_randomForestSRC_cvdDeath.csv",row.names=F,quote=F, sep=",")
write.table(results_cancer, "aric_EA_randomForestSRC_cancerDeath.csv",row.names=F,quote=F, sep=",")
